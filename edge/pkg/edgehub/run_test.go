...
func TestRunReturnsWhenDoneAlreadyClosed(t *testing.T) {
	...
	close(done)
	waitFor(t, h.start(done), "run to return")
	if h.inits.Load() != 0 {
...
func TestRunRetriesInitAfterBackoff(t *testing.T) {
	...
	started := time.Now()
	close(done)
	waitFor(t, exited, "run to return during the backoff wait")
...
func TestRunReconnectsOnTransportError(t *testing.T) {
	...
	exited := h.start(done)
	waitFor(t, h.connected, "the first connect")
	waitFor(t, h.uninited, "UnInit after the transport error")
	uninitedAt := time.Now()
	waitFor(t, h.connected, "the reconnect after the backoff")
	// reconnectBackoff starts at 2s (+ up to 20% jitter); anything shorter
	...
	waitFor(t, h.uninited, "UnInit after the second transport error")

	started := time.Now()
	close(done)
	waitFor(t, exited, "run to return during the reconnect backoff")
	if time.Since(started) > time.Second {
		t.Fatalf("run took %v to stop; it must not sleep out the backoff after done is closed", time.Since(started))
	}
...
func TestRunReconnectsImmediatelyOnRotation(t *testing.T) {
	...
	waitFor(t, h.uninited, "UnInit after the rotation signal")
	rotatedAt := time.Now()
	waitFor(t, h.connected, "the reconnect after rotation")
	if d := time.Since(rotatedAt); d > time.Second {
		t.Fatalf("reconnect after rotation took %v; it must not wait a backoff interval", d)
	}

	close(done)
	waitFor(t, h.uninited, "UnInit on shutdown while connected")
	waitFor(t, exited, "run to return after done")
	if got := h.inits.Load(); got != 2 {
...
func TestRunDropsStaleRotationBeforeConnect(t *testing.T) {
	...
	exited := h.start(done)
	waitFor(t, h.connected, "the connect")
